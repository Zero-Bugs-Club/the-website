...
import { getMemberBySlug } from '../config/about.config';
...
const ProfileAvatar = ({ name }) => {
    ...
    return (
        <div className="h-48 w-48 rounded-3xl border border-white/10 bg-gradient-to-br from-neutral-700 via-neutral-900 to-black flex items-center justify-center text-5xl font-bold text-white shadow-2xl sm:h-56 sm:w-56">
            {initials}
        </div>
    );
};
...
const MemberProfilePage = () => {
    const { deptName, memberName } = useParams();
    const { department, member } = getMemberBySlug(deptName, memberName);
        ...
        return (
            <main className="min-h-screen bg-black px-4 pt-32 text-white">
                <div className="mx-auto max-w-4xl">
                    <Link 
                        to={department ? `/about/${department.slug}` : '/about'} 
                    ...
                    <h1 className="text-4xl font-bold tracking-tighter">Member profile not found</h1>
                    <p className="mt-4 text-gray-400">This member profile has not been added yet.</p>
                </div>
            </main>
        );
    }
        ...
        <main className="relative min-h-screen overflow-hidden bg-black px-4 pb-20 pt-24 text-white sm:px-6 lg:px-8">
            ...
            <div className="relative z-10 mx-auto max-w-6xl">
                ...
                <section className="grid gap-10 border-b border-white/10 pb-12 lg:grid-cols-[auto_1fr] lg:items-end">
                    ...
                    <div>
                        <p className="mb-4 font-mono text-sm uppercase tracking-[0.3em] text-gray-500">{department.name} Department</p>
