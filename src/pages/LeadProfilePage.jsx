        ...
        return (
            <main className="min-h-screen bg-black px-4 pt-32 text-white">
                <div className="mx-auto max-w-4xl">
                    ...
                    <h1 className="text-4xl font-bold tracking-tighter">Lead profile not found</h1>
                    <p className="mt-4 text-gray-400">This lead profile has not been added yet.</p>
                </div>
            </main>
        );
    }
        ...
        <main className="relative min-h-screen overflow-hidden bg-black px-4 pb-20 pt-24 text-white sm:px-6 lg:px-8">
            ...
            <div className="relative z-10 mx-auto max-w-6xl">
                ...
                <section className="grid gap-10 border-b border-white/10 pb-12 lg:grid-cols-[auto_1fr] lg:items-end">
                    <ProfileAvatar name={lead.name} />
                    <div>
                        <p className="mb-4 font-mono text-xl uppercase tracking-[0.3em] text-gray-500">{department.name} </p>
                        <h1 className="text-5xl font-bold tracking-tighter text-white md:text-8xl">{lead.name}</h1>
                        <p className="mt-3 font-mono text-xl uppercase tracking-[0.24em] text-blue-400">{lead.role}</p>
                        <p className="mt-6 max-w-3xl text-lg leading-relaxed text-gray-400">{lead.description}</p>
                        <div className="mt-8 flex gap-3">
                            <SocialLink href={lead.github} label="GitHub" icon={<Github size={18} />} />
                            <SocialLink href={lead.linkedin} label="LinkedIn" icon={<Linkedin size={18} />} />
                            <SocialLink href={lead.instagram} label="Instagram" icon={<Instagram size={18} />} />
                        </div>
                    </div>
                </section>
                ...
                <section className="grid gap-8 py-12 lg:grid-cols-[0.8fr_1.2fr]">
                    <div>
                        <h2 className="mt-3 text-3xl font-bold tracking-tight text-white">Lead Note</h2>
                    </div>
                    <div className="rounded-2xl border border-white/10 bg-gradient-to-b from-white/10 to-black/20 p-8 backdrop-blur-sm">
                        <p className="text-gray-300 leading-relaxed">{department.leadNote}</p>
                    </div>
                </section>
                ...
                <section>
                    ...
                    <div className="grid gap-4">
                        {lead.contributions.length === 0 ? (
                            <div className="text-gray-400">No contributions listed.</div>
                        ) : (
                            lead.contributions.map((contribution, index) => (
                                <div
                                    key={contribution}
